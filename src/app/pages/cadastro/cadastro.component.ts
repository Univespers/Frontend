...
export class CadastroComponent implements AfterContentInit, RequiresSave {
  ...
  @ViewChild("cadastro") cadastro?: NgForm;
  ...
  nextStep() {
    if(this.isLastStep()) {
      this.cadastro?.ngSubmit.emit(); // Trigger submit
  ...
  submit(userDataForm: any, userContactsForm: any, userCourseForm: any) {
    ...
    // Register User
    of(true).pipe(
      switchMap(data => this.authService.cadastro(institutionalEmail, password)),
      switchMap(data => this.authService.login(institutionalEmail, password)),
      switchMap(data => this.perfilService.createPerfil(name, institutionalEmail, course, pole)),
